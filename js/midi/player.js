...
//				console.log(event.channel, MIDI.defineControl[event.controllerType], event.value);
			...
			case 'noteOn':
				...
				if (!instruments[gm.id]) console.log(gm);
...
midi.getAllInstruments = function() {
	var instruments = {};
	var programs = {};
	for (var n = 0; n < 127; n ++) {
		var gm = MIDI.GM.byId[n];
...
function setStartInterval(start) {
	let now = window.performance.now();

	midi.currentTime+=now-start;
	if (midi.currentTime < 0 ) {
		window.requestAnimationFrame(function() {
			setStartInterval(now)
		});	
	} else {
		startAudio(midi.currentTime, null, null);
		...
		switch (event.subtype) {
			...
			case 'noteOn':
				...
				eventQueue.push({
				    event: event,
				    time: queueTime,
				    source: MIDI.noteOn(channelId, event.noteNumber, event.velocity, delay, trackId),
