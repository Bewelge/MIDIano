...
function parseMidi(data) {
 ...
 let bpm =   Parser.setTemporal(midiData)
  //Parser.setAbsoluteTimestamps(midiData)
  return {
    header: header,
    tracks: tracks,
    bpm: bpm
...
class Parser {
  ...
  static setAbsoluteTimestamps(midiObj) {
    //Add deltatime up to get absolute timestamps
    for (let t in midiObj.tracks) {
      let time = 0;
      for (let n in midiObj.tracks[t]) {
        let note = midiObj.tracks[t][n]
        time += note.temporalDelta
      //  note.timestamp = time
      }
    }
  }
